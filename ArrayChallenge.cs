...
class MainClass {
  ...
  public static string ArrayChallenge(string[] strArr) {
	...
	static void Main() {  
		// Prompt the user to input the grid
		Console.WriteLine("Enter the grid (each row separated by comma): ");
		string input = Console.ReadLine();

		// Split the input into rows using commas
		string[] rows = input.Split(',');

		// Remove any unnecessary characters from each row
		for (int i = 0; i < rows.Length; i++) {
			rows[i] = rows[i].Replace("\"", "").Trim();
		}

		// Call ArrayChallenge and print the result
		Console.WriteLine("------------------------------");
		Console.WriteLine(ArrayChallenge(rows));
	} 
}